...
dst_split_plot <- function(.eventid, n_skiers = 30, n_seg = 6,
  ...
  title <- paste(
    race_info$date, race_info$location, race_info$gender,
  ...
  dst_race <- tbl(
    ...
    group_by(split_km) |>
    mutate(
      time_back = split_time - ref_split_fun(split_time, na.rm = TRUE, ref_split),
  ...
  p_bottom <- p_bottom +
    scale_x_continuous(breaks = unique(dst_race$split_km), expand = expansion(mult = 0.1)) +
    labs(x = "Km", y = "Position", caption = "statisticalskier.netlify.app") +
    theme_bw()
...
dst_pur_split_plot <- function(.eventid, n_skiers = 30, n_seg = 6,
  ...
  dst_race <- dst_race |>
    group_by(compid) |>
    mutate(
      split_km = if_else(race_order == 2L,
