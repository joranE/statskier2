...
dst_hth_data <- function(ath_compid, opp_compid) {
  ...
  sql <- read_sql(system.file("sql/skier_head_to_head_dst.sql",package = "statskier2"))
...
spr_hth_data <- function(ath_compid, opp_compid) {
  ...
  sql <- read_sql(system.file("sql/skier_head_to_head_spr.sql",package = "statskier2"))
  sql <- glue::glue_sql(sql,
    ath_compid = ath_compid,
    opp_compid = opp_compid,
    .con = ..statskier_pg_con..
...
spr_heats_hth_data <- function(ath_compid, opp_compid) {
  ...
  sql <- read_sql(system.file("sql/skier_head_to_head_spr_heats.sql",package = "statskier2"))
  sql <- glue::glue_sql(sql,
    ath_compid = ath_compid,
    opp_compid = opp_compid,
    .con = ..statskier_pg_con..
