...
dev_cohort_dst_qgam <- function(top_k = 10,
  ...
  if (type == "fispoints") {
    ...
    tune_lsig_men <- qgam::tuneLearnFast(
      form = fispoints ~ s(age),
      data = tuning_data_men,
      qu = q,
      control = list(progress = FALSE)
    )
    tune_lsig_wom <- qgam::tuneLearnFast(
      form = fispoints ~ s(age),
    ...
    dst_mod_men <- qgam::mqgam(
      form = fispoints ~ s(age),
      data = dst_result_men,
      qu = q,
      lsig = tune_lsig_men$lsig
    )
    dst_mod_wom <- qgam::mqgam(
      form = fispoints ~ s(age),
      data = dst_result_wom,
      qu = q,
  ...
  if (type == "pbm_pts") {
    ...
    tuning_data <- dst_result |>
      slice_sample(prop = 0.1)
    message("Estimating lsig...")
    tune_lsig <- qgam::tuneLearnFast(
      form = pbm_pts ~ s(age, by = gender),
      data = tuning_data,
      qu = q,
      control = list(progress = FALSE)
    )
  ...
  new_data_dst_men <- data.frame(
    age = seq(18, 45, by = 0.5),
    gender = "Men"
  )
  new_data_dst_wom <- data.frame(
    age = seq(18, 45, by = 0.5),
    gender = "Women"
  )
  new_data_dst_men$q_upper <- qgam::qdo(dst_mod_men,
    qu = q[2],
    predict,
    newdata = new_data_dst_men
  )
  new_data_dst_men$q_lower <- qgam::qdo(dst_mod_men,
    qu = q[1],
    predict,
    newdata = new_data_dst_men
  )

  new_data_dst_wom$q_upper <- qgam::qdo(dst_mod_wom,
    qu = q[2],
    predict,
    newdata = new_data_dst_wom
  )
  new_data_dst_wom$q_lower <- qgam::qdo(dst_mod_wom,
    qu = q[1],
    predict,
    newdata = new_data_dst_wom
  )
...
dev_cohort_spr_qgam <- function(top_k = 12,
  ...
  if (type == "fispoints") {
    ...
    message("Fitting full model...")
    spr_mod_men <- qgam::mqgam(
      form = fispoints ~ s(age),
      data = spr_result_men,
      qu = q,
      lsig = tune_lsig_men$lsig
    )
    spr_mod_wom <- qgam::mqgam(
      form = fispoints ~ s(age),
      data = spr_result_wom,
      qu = q,
      lsig = tune_lsig_wom$lsig
  ...
  new_data_spr_men <- data.frame(
    age = seq(18, 45, by = 0.5),
    gender = "Men"
  )
  new_data_spr_wom <- data.frame(
    age = seq(18, 45, by = 0.5),
    gender = "Women"
  )
  new_data_spr_men$q_upper <- qgam::qdo(spr_mod_men,
    qu = q[2],
    predict,
    newdata = new_data_spr_men
  )
  new_data_spr_men$q_lower <- qgam::qdo(spr_mod_men,
    qu = q[1],
    predict,
    newdata = new_data_spr_men
  )

  new_data_spr_wom$q_upper <- qgam::qdo(spr_mod_wom,
    qu = q[2],
    predict,
    newdata = new_data_spr_wom
  )
  new_data_spr_wom$q_lower <- qgam::qdo(spr_mod_wom,
    qu = q[1],
    predict,
    newdata = new_data_spr_wom
  )
